...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.criterionCycle = MS_SSIM_L1_loss.MS_SSIM_L1_LOSS()
            #self.criterionCycle = torch.nn.L1Loss()
            #self.criterionIdt=torch.nn.L1Loss
            self.criterionIdt = MS_SSIM_L1_loss.MS_SSIM_L1_LOSS()
            #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            #loss_func = nn.MSELoss().to(device)
            #layer_indexs = [3, 8, 15, 22]
            #self.criterionIdt = Perceptual_loss(loss_func, layer_indexs, device)
            #self.criterionCycle = Perceptual_loss(loss_func, layer_indexs, device)
            #self.criterionIdt = MS_SSIM_L1_loss.MS_SSIM_L1_LOSS()
